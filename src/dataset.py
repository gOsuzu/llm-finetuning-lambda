...
def clean_dialogue(client, text, system_prompt):
    """Clean a single dialogue with the OpenAI API.

    This function uses the OpenAI API to clean a single dialogue transcript.
    It applies the provided system prompt to the dialogue and returns the cleaned text.
...
def clean_conversations(dataset):
    """clean all conversations in the dataset by removing action discriptions and context.
...
def main():
    ...
    cleaned_dataset.push_to_hub(
        "gOsuzu/rick-and-morty-transcripts-sharegpt",
